...
# Vortex integration tests for add_files functionality
def test_add_vortex_files_to_unpartitioned_table(spark: SparkSession, session_catalog: Catalog, format_version: int) -> None:
    """Test adding Vortex files to an unpartitioned Iceberg table."""
    ...
    identifier = f"default.test_add_vortex_files_unpartitioned_v{format_version}"
    
    tbl = _create_table(session_catalog, identifier, format_version=format_version)

    # Create Vortex test files
    vortex_file_1 = "s3://warehouse/default/test_add_vortex_files/data_1.vortex"
    vortex_file_2 = "s3://warehouse/default/test_add_vortex_files/data_2.vortex"
    
    # Write Vortex files
    io = FsspecFileIO()
    data_1 = ARROW_TABLE.slice(0, 1)  # First row
    write_vortex_file(data=data_1, file_path=vortex_file_1, io=io)
    
    data_2 = ARROW_TABLE.slice(1, 1)  # Second row
    write_vortex_file(data=data_2, file_path=vortex_file_2, io=io)

    # Add Vortex files to table
    tbl.add_files(file_paths=[vortex_file_1, vortex_file_2])

    # Verify the files were added with Vortex format
...
def test_add_mixed_format_files_to_table(spark: SparkSession, session_catalog: Catalog, format_version: int) -> None:
    ...
    identifier = f"default.test_add_mixed_format_files_v{format_version}"
    
    tbl = _create_table(session_catalog, identifier, format_version=format_version)

    # Create files in different formats
    vortex_file = "s3://warehouse/default/test_mixed_formats/data.vortex"
    parquet_file = "s3://warehouse/default/test_mixed_formats/data.parquet"
    
    # Write Vortex file
    io = FsspecFileIO()
    vortex_data = ARROW_TABLE.slice(0, 1)
    write_vortex_file(data=vortex_data, file_path=vortex_file, io=io)
    
    # Write Parquet file
    parquet_data = ARROW_TABLE.slice(1, 1)
    fo = tbl.io.new_output(parquet_file)
    with fo.create(overwrite=True) as fos:
        with pq.ParquetWriter(fos, schema=ARROW_SCHEMA) as writer:
            writer.write_table(parquet_data)
