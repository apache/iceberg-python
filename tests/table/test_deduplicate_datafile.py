...
def test_deduplicate_ensures_no_duplicate_paths_across_all_snapshots(
    iceberg_catalog: InMemoryCatalog, tmp_path: Path
) -> None:
    ...
    try:
        ...
        assert len(duplicates_remaining) == 0, (
            f"Found duplicate file paths after deduplication: {duplicates_remaining}"
        )
