...
class TestSnapshotUtilities:
    ...
    def create_mock_table_metadata(self) -> TableMetadata:
        ...
        metadata = Mock()  # Remove spec to allow attribute assignment
        metadata.refs = {}
        metadata.snapshots = []
    ...
    def test_is_fast_forward_possible(self) -> None:
        ...
        def mock_snapshot_by_id(snapshot_id: int) -> Snapshot | None:
            if snapshot_id == 100:
        ...
        metadata.snapshot_by_id.side_effect = mock_snapshot_by_id  # type: ignore[attr-defined]
    ...
    def test_find_common_ancestor(self) -> None:
        ...
        snapshots = {
            1: Snapshot(
                snapshot_id=1, parent_snapshot_id=None, manifest_list="s3://1.avro", timestamp_ms=1000, sequence_number=1
            ),
        ...
        metadata.snapshot_by_id.side_effect = lambda sid: snapshots.get(sid)  # type: ignore[attr-defined]
    ...
    def test_find_common_ancestor_no_common_ancestor(self) -> None:
        ...
        snapshots = {
            ...
            2: Snapshot(snapshot_id=2, parent_snapshot_id=1, manifest_list="s3://2.avro", timestamp_ms=2000, sequence_number=2),
            10: Snapshot(
                snapshot_id=10, parent_snapshot_id=None, manifest_list="s3://10.avro", timestamp_ms=1000, sequence_number=1
            ),
            11: Snapshot(
                snapshot_id=11, parent_snapshot_id=10, manifest_list="s3://11.avro", timestamp_ms=2000, sequence_number=2
            ),
        }
    ...
    def test_find_common_ancestor_missing_snapshots(self) -> None:
        ...
        # Mock already configured to return None for missing snapshots
...
class TestSquashMergeStrategy:
    ...
    def test_squash_merge_fast_forward(self) -> None:
        ...
        def mock_snapshot_by_id(snapshot_id: int) -> Snapshot | None:
            if snapshot_id == 200:
                return Snapshot(
                    snapshot_id=200, parent_snapshot_id=100, manifest_list="s3://200.avro", timestamp_ms=2000, sequence_number=2
                )
            elif snapshot_id == 100:
                return Snapshot(
                    snapshot_id=100, parent_snapshot_id=None, manifest_list="s3://100.avro", timestamp_ms=1000, sequence_number=1
                )
            return None

        transaction.table_metadata.snapshot_by_id.side_effect = mock_snapshot_by_id
    ...
    def test_squash_merge_divergent_branches(self) -> None:
        ...
        def mock_snapshot_by_id(snapshot_id: int) -> Snapshot | None:
            snapshots = {
                ...
                300: Snapshot(
                    snapshot_id=300, parent_snapshot_id=100, manifest_list="s3://300.avro", timestamp_ms=3000, sequence_number=3
                ),
            }
...
class TestManageSnapshotsMergeBranch:
    ...
    def test_merge_branch_with_custom_message(self) -> None:
        ...
        with patch("pyiceberg.table.update.snapshot._get_merge_strategy_impl") as mock_strategy:
            ...
            mock_impl.merge.assert_called_with(
                source_branch="feature", target_branch="main", transaction=transaction, merge_commit_message=custom_message
            )
    ...
    def test_merge_branch_with_source_deletion(self) -> None:
        ...
        with (
            patch("pyiceberg.table.update.snapshot._get_merge_strategy_impl") as mock_strategy,
            patch.object(manage_snapshots, "_remove_ref_snapshot", return_value=((Mock(),), (Mock(),))) as mock_remove,
        ):
            mock_impl = Mock()
            mock_impl.merge.return_value = ((Mock(),), (Mock(),))
            mock_strategy.return_value = mock_impl
            ...
            mock_impl.merge.assert_called_with(
                source_branch="feature", target_branch="main", transaction=transaction, merge_commit_message=None
            )

            # Should also call remove source branch
            mock_remove.assert_called_with(ref_name="feature")
    ...
    def test_all_merge_strategies_with_source_deletion(self) -> None:
        ...
        with patch.object(manage_snapshots, "_remove_ref_snapshot", return_value=((Mock(),), (Mock(),))) as mock_remove:
            strategies = [
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_strategy_with_missing_snapshots(self) -> None:
        ...
        target_ref = SnapshotRef(snapshot_id=888, snapshot_ref_type=SnapshotRefType.BRANCH)
        transaction.table_metadata.refs = {"feature": source_ref, "main": target_ref}
        transaction.table_metadata.snapshot_by_id = Mock()
        transaction.table_metadata.snapshot_by_id.return_value = None
...
class TestIntegrationAndBehavioralDifferences:
    ...
    def test_strategy_selection_and_execution(self) -> None:
        ...
        with (
            patch("pyiceberg.table.update.snapshot._SquashMergeStrategy") as mock_squash,
...
def test_branch_merge_strategy_enum() -> None:
...
def test_get_merge_strategy_impl() -> None:
